...
function expenseId(req: Request) {
  ...
  if (typeof value !== "string") throw new ApiError(400, "Invalid expense ID.");
  return value;
}

export async function create(req: Request, res: Response) {
  const expense = await createHouseholdExpense(req.user!.id, householdId(req), req.body);
...
export async function getById(req: Request, res: Response) {
  const expense = await getHouseholdExpense(req.user!.id, householdId(req), expenseId(req));
